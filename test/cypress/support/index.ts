...
Cypress.Commands.add('attachIntercepts', () => {
  cy.intercept(/api.what3words.com\/v3\/autosuggest/i)
    .as('autosuggest')
    .intercept(/utilisation-api.what3words.com\/autosuggest-session/i, (req) =>
      req.reply(200)
    )
    .as('utilisation')
    .intercept('POST', /wp-login.php/i)
    .as('login-request')
    .intercept(/wp-admin\/plugin-install.php/i)
    .as('plugin-install-page')
    .intercept(/wp-admin\/admin-ajax.php/i)
    .as('ajax')
    .intercept(/wp-admin\/plugins.php/i)
    .as('plugins-page');
});
...
Cypress.Commands.add(
  'installPlugin',
  (plugin_name: string, plugin_slug: string) => {
    cy.visit('/wp-admin/plugin-install.php')
      .get('#search-plugins')
      .type(plugin_name)
      .wait('@ajax')
      .get(`a[data-slug="${plugin_slug}"]`)
      .click()
      .wait('@ajax');
  }
);
...
Cypress.Commands.add(
  'activatePlugin',
  (plugin_name: string, root_php_file?: string) => {
    cy.visit('/wp-admin/plugins.php')
      .get(
        `[data-plugin="${plugin_name}/${
          root_php_file || plugin_name
        }.php"] .activate > a`
      )
      .click()
      .get(
...
Cypress.Commands.add('setApiKey', (api_key: string) => {
  cy.intercept('POST', /admin.php/i)
    .as('submit')
    .getElementByDataTestId('api_key')
    .focus()
    .type(api_key)
    .getElementByDataTestId('save_api_key')
    .should('be.visible')
    .click({ force: true }) // Forcing to ensure it is clickable
    .wait('@submit')
    .getElementByDataTestId('api_key')
...
Cypress.Commands.add('setSelector', (selector: string) => {
  cy.intercept('POST', /admin.php/i)
    .as('submit')
    .getElementByDataTestId('enable_input_selector')
    .click()
    .getElementByDataTestId('input_selector')
    .type(selector)
    .getElementByDataTestId('save_settings')
    .should('be.visible')
    .click({ force: true }) // Forcing to ensure it is clickable
    .wait('@submit')
    .getElementByDataTestId('input_selector')
    .should('have.value', selector);
});

Cypress.Commands.add('assertApiKeyNotice', (exists: boolean) => {
  if (exists) {
    cy.getElementByDataTestId('api_key_notice')
      .should('exist')
      .should('be.visible')
      .getElementByDataTestId('get_api_key')
      .should('exist')
      .should('be.visible')
      .getElementByDataTestId('manage_api_key')
      .should('not.exist');
  } else {
    cy.getElementByDataTestId('api_key_notice')
      .should('not.exist')
...
Cypress.Commands.add(
  'assertTargetValidity',
  (test_id: string, validity: boolean) => {
...
Cypress.Commands.add(
  ...
  (
    ...
    cy.intercept(/api.what3words.com\/v3\/autosuggest/i)
      ...
      .clear()
      .type(first)
      .get(`${fieldPrefix}last_name`)
      .focus()
      .clear()
      .type(last)
      .get(`${fieldPrefix}city`)
      .focus()
      .clear()
      .type(city)
      .get(`${fieldPrefix}postcode`)
      .focus()
      .clear()
      .type(postcode)
      .get(`${selectPrefix}country-container`)
      .click()
      .get('li')
...
Cypress.Commands.add('saveSettings', () => {
  cy.getElementByDataTestId('save_settings')
    .should('be.visible')
    .click({ force: true });
});

Cypress.Commands.add('saveAdvanced', () => {
  cy.getElementByDataTestId('save_advanced')
    .should('be.visible')
    .click({ force: true });
});
...
Cypress.Commands.add('selectManagedInput', () => {
  cy.getElementByDataTestId('enable_w3w_managed')
    .should('be.visible')
    .click({ force: true });
});

Cypress.Commands.add('selectInputSelector', () => {
  cy.getElementByDataTestId('enable_input_selector')
    .should('be.visible')
    .click({ force: true });
});
...
Cypress.Commands.add('toggleLabel', () => {
  cy.getElementByDataTestId('enable_label')
    .should('be.visible')
    .click({ force: true });
});

Cypress.Commands.add('toggleAdvancedSettings', () => {
  cy.getElementByDataTestId('advanced_dropdown')
    .should('be.visible')
    .click({ force: true });
});

Cypress.Commands.add('togglePlaceholder', () => {
  cy.getElementByDataTestId('enable_placeholder')
    .should('be.visible')
    .click({ force: true });
});

Cypress.Commands.add('toggleClipToCountry', () => {
  cy.getElementByDataTestId('enable_clip_to_country')
    .should('be.visible')
    .click({ force: true });
});

Cypress.Commands.add('toggleClipToCircle', () => {
  cy.getElementByDataTestId('enable_clip_to_circle')
    .should('be.visible')
    .click({ force: true });
});

Cypress.Commands.add('toggleClipToBoundingBox', () => {
  cy.getElementByDataTestId('enable_clip_to_bounding_box')
    .should('be.visible')
    .click({ force: true });
});

Cypress.Commands.add('toggleClipToPolygon', () => {
  cy.getElementByDataTestId('enable_clip_to_polygon')
    .should('be.visible')
    .click({ force: true });
});
